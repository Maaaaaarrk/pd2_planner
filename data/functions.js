...
function loadParams() {
	//if (params.has('level') == true) {		// if level is a parameter, all parameters are checked
		...
		var param_url = 1;//~~params.get('url');
...
function corrupt(group, val) {
    ...
    var baseGroup = isMerc ? group.slice(5) : group;
    var stats = isMerc ? mercenary : character;
    var itemEquipped = isMerc ? (mercEquipped[baseGroup] || {}) : equipped[group];
    ...
    if (val == "­ ­ ­ ­ Corruption" || val == "none" || val == group || itemEquipped.rarity == "rw" || itemEquipped.rarity == "common" || (group == "offhand" && itemEquipped.type != "quiver" && equipped.weapon.twoHanded == 1 && (equipped.weapon.type != "sword" || character.class_name != "Barbarian"))) {
        ...
        for (outcome in corruptions[group]) {
            if (corruptions[group][outcome].name == val && (group != "offhand" || (offhandType == corruptions[group][outcome].base || offhandType == "none"))) {
                ...
                if (corruptions[group][outcome]["name"].includes("Sockets")) {
                    ...
                    unlimitSockets = 1;
                } else if (group != "offhand" && group != "weapon" && group != "merc_offhand" && group != "merc_weapon") {
                    unlimitSockets = 1;
                }
                for (affix in corruptions[group][outcome]) {
                    ...
                    } else if (affix == "sockets" && (group == "weapon" || group == "offhand" || group == "merc_weapon" || group == "merc_offhand") && unlimitSockets == 1 && (typeof itemEquipped.twoHanded == 'undefined' || itemEquipped.twoHanded != 1)) {
                        if (typeof itemEquipped.type !== 'undefined' && itemEquipped.type === "shield") {
                            if (typeof stats[affix] == 'undefined') stats[affix] = 0;
                            stats[affix] += 3
                            corruptsEquipped[group][affix] = 3
                        } else if ((group == "offhand" || group == "merc_offhand") && typeof itemEquipped.type !== 'undefined' && itemEquipped.type === "quiver") {
                            if (typeof stats[affix] == 'undefined') stats[affix] = 0;
                            stats[affix] += 2
                            corruptsEquipped[group][affix] = 2
                        } else {
...
function equipSwap(group, val) {
	...
	// for offhand, check if item is in the weapon array (barbarian/assassin weapons in offhand)
	var src_group = group;
	if (group == "offhand") {
		var found = 0;
...
function itemHover(ev, id) {
	...
	if (type === "charm") {
        // Safely check if equipped["charms"] and equipped["charms"][val] exist
        if (equipped && equipped["charms"] && equipped["charms"][val]) {
            let charm = equipped["charms"][val];
            ...
            if (charm.size !== "small" && charm.size !== "large" && charm.size !== "grand") {
                color = "Red";
            }
            ...
            if (typeof name === "string" && name.substr(0, 3) === "+1 " && height === 3) {
                name = name.substr(3);
            }

            if (charm.rarity === "magic" || charm.debug === 1) {
                base = "";
            }
        }
    } else {
	...
	let cell_x = 9; // default
    let cell_y = 0; // default

    if (typeof id === "string") {
        if (id.length > 1) {
            cell_x = parseInt(id[1], 10) - 1;
            if (cell_x === -1) {
                cell_x = 9;
            }
        }
...
function equipSwapHover(group) {
	...
	document.getElementById("item_name").innerHTML = name+base+runeword
	document.getElementById("item_info").innerHTML = main_affixes
	document.getElementById("item_corruption").innerHTML = ""
	document.getElementById("item_affixes").innerHTML = affixes
	document.getElementById("item_set_affixes").innerHTML = ""
	...
	if (name === "") {
		...
		document.getElementById("tooltip_inventory").style.display = "block"
		// Position tooltip relative to inventory_container using dynamic bounding rects
...
function socket(event, group, source) {
	...
	if (spaceFound == 1) {
		...
		var groups = ["helm", "armor", "weapon", "offhand", "amulet", "belt", "merc_helm", "merc_armor", "merc_weapon", "merc_offhand", "merc_belt"];
		for (let g = 0; g < groups.length; g++) {
			...
			for (let i = 0; i < socketed[groups[g]].items.length; i++) {
				if (inv[0].onpickup == socketed[groups[g]].items[i].id) {
					for (affix in socketed[groups[g]].items[i]) { if (affix != "id") {
						groupStats[affix] -= socketed[groups[g]].items[i][affix]
						socketed[groups[g]].totals[affix] -= socketed[groups[g]].items[i][affix]
					} }
					socketed[groups[g]].items[i] = {id:"",name:""}
					socketed[groups[g]].socketsFilled -= 1
		...
		for (let k = 0; k < socketables.length; k++) { if (socketables[k].name == name) {
			...
			for (affix in socketables[k]) {
				if (affix != "type" && affix != "rarity" && affix != "img") {
					...
					if (affix == "e_damage") { if (group == "weapon" || group == "merc_weapon" || (group == "offhand" && offhandType == "weapon") || group == "merc_offhand") { /*don't change*/ } else { affix_dest = "damage_bonus" } }
					...
					if (typeof stats[affix_dest] == 'undefined') stats[affix_dest] = 0;
					stats[affix_dest] += socketables[k][affix]
					socketed[group].totals[affix_dest] += socketables[k][affix]
				}
				if (affix == baseGroup || (affix == "armor" && baseGroup == "helm") || (affix == "armor" && baseGroup == "offhand" && typeof(socketables[k]["shield"]) == 'undefined' && offhandType != "weapon") || (affix == "shield" && baseGroup == "offhand" && offhandType != "weapon") || (affix == "weapon" && baseGroup == "offhand" && offhandType == "weapon")) {
					for (groupAffix in socketables[k][affix]) {
						...
						if (typeof stats[groupAffix] == 'undefined') stats[groupAffix] = 0;
						stats[groupAffix] += socketables[k][affix][groupAffix]
...
function allowSocket(event, group) {
	var isMerc = group.startsWith("merc_");
	var itemEquipped = isMerc ? mercEquipped[group.slice(5)] : equipped[group];
...
function updateSocketTotals() {
	var groups = ["helm", "armor", "weapon", "offhand", "belt", "amulet", "merc_helm", "merc_armor", "merc_weapon", "merc_offhand", "merc_belt"];
	for (let g = 0; g < groups.length; g++) {
